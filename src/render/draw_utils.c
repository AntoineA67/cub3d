...
/*   Updated: 2022/06/11 14:36:11 by qroussea         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_square_texture_center(t_vars *v, t_coord p, t_data *img, double di)
{
	int		dy;
	int		dx;
	double	img_x;
	...
	while (++dy < p.y + p.z / 2)
	{
		...
		while (++dx < p.x + p.z / 2 && img_x < img->size.x && img_y >= 0
			...
			&& is_in_window(v, dx, dy - v->player.rot.y - v->jump_height / di))
		{
			pixel_put(v->img, dx, dy - v->player.rot.y - v->jump_height / di,
