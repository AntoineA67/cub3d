...
/*   Updated: 2022/05/17 15:11:39 by arangoni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++i < vars->max_n_enemies)
	{
		if (vars->enemies[i].lives > 0 && ts - vars->enemies[i].last_attack > 1000)
		{
			...
			if (dist_enemy < ray.dist)
			{
				...
				new_pos.x = vars->enemies[i].pos.x + ((.08 - .16 * (vars->player.pos.x < vars->enemies[i].pos.x)) * vars->delta_time * 20.0 * (fabs(vars->enemies[i].pos.x - vars->player.pos.x) > 1.0));
				new_pos.y = vars->enemies[i].pos.y + ((.08 - .16 * (vars->player.pos.y < vars->enemies[i].pos.y)) * vars->delta_time * 20.0 * (fabs(vars->enemies[i].pos.y - vars->player.pos.y) > 1.0));
			...
			{
				new_pos.x = vars->enemies[i].pos.x + ((.08 - .16 * (vars->enemies[i].last_player_pos.x < vars->enemies[i].pos.x)) * vars->delta_time * 20.0 * (fabs(vars->enemies[i].pos.x - vars->enemies[i].last_player_pos.x) > 1.0));
				new_pos.y = vars->enemies[i].pos.y + ((.08 - .16 * (vars->enemies[i].last_player_pos.y < vars->enemies[i].pos.y)) * vars->delta_time * 20.0 * (fabs(vars->enemies[i].pos.y - vars->enemies[i].last_player_pos.y) > 1.0));
				// printf("%.2f %.2f	%.2f %.2f\n", new_pos.x, new_pos.y, fabs(vars->enemies[i].pos.x - vars->player.pos.x), fabs(vars->enemies[i].pos.y - vars->player.pos.y));
				change_case(vars, new_pos.x, new_pos.y, &vars->enemies[i].pos);
				vars->enemies[i].pos.x = new_pos.x;
				vars->enemies[i].pos.y = new_pos.y;
